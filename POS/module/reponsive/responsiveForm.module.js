...
//	layout.verticalMargin = this._originalLayout.verticalMargin;
	layout.topMargin = this._originalLayout.topMargin;
	layout.rightMargin = this._originalLayout.rightMargin;
	layout.bottomMargin = this._originalLayout.bottomMargin;
	layout.leftMargin = this._originalLayout.leftMargin;
	...
	// 20221213 modified by shinhs95 → "hide-on-mobile"과 "hide-on-tablet" 속성에 따라 변수가 달라지기 때문에 미리 선언
...
//				layout.horizontalMargin = this._container.userAttr(ATTR_MOBILE_H_MARGIN);
				layout.rightMargin = this._container.userAttr(ATTR_MOBILE_H_MARGIN);
				layout.leftMargin = this._container.userAttr(ATTR_MOBILE_H_MARGIN);
				layout.horizontalSpacing = parseInt(this._container.userAttr(ATTR_MOBILE_H_MARGIN)) * 2;
			}
			if (this._container.userAttr(ATTR_MOBILE_V_MARGIN)) {
//				layout.verticalMargin = this._container.userAttr(ATTR_MOBILE_V_MARGIN);
				layout.topMargin = this._container.userAttr(ATTR_MOBILE_V_MARGIN);
				layout.bottomMargin = this._container.userAttr(ATTR_MOBILE_V_MARGIN);
				layout.verticalSpacing = parseInt(this._container.userAttr(ATTR_MOBILE_V_MARGIN)) * 2;
			}
			
			// 20221213 modified by shinhs95 → "hide-on-mobile" 속성 사용 시 적용
			visibleChildren = sortedChildren.filter(function( /* cpr.controls.VisibleUIControl */ each) {
				return each.userAttr(ATTR_HIDE_ON_MOBILE) != "true";
			});
			...
			hiddenChildren.forEach(function( /* cpr.controls.VisibleUIControl */ each) {
				each.visible = false;
			});
...
//				layout.horizontalMargin = this._container.userAttr(ATTR_TABLET_H_MARGIN);
				layout.rightMargin = this._container.userAttr(ATTR_TABLET_H_MARGIN);
				layout.leftMargin = this._container.userAttr(ATTR_TABLET_H_MARGIN);
				layout.horizontalSpacing = parseInt(this._container.userAttr(ATTR_TABLET_H_MARGIN)) * 2;
			}
			if (this._container.userAttr(ATTR_TABLET_V_MARGIN)) {
//				layout.verticalMargin = this._container.userAttr(ATTR_TABLET_V_MARGIN);
				layout.topMargin = this._container.userAttr(ATTR_TABLET_V_MARGIN);
				layout.bottomMargin = this._container.userAttr(ATTR_TABLET_V_MARGIN);
				layout.verticalSpacing = parseInt(this._container.userAttr(ATTR_TABLET_V_MARGIN)) * 2;
			}
			
			// 20221213 modified by shinhs95 → "hide-on-tablet" 속성 사용 시 적용
			visibleChildren = sortedChildren.filter(function( /* cpr.controls.VisibleUIControl */ each) {
				return each.userAttr(ATTR_HIDE_ON_TABLET) != "true";
			});
	...
	// 20221213 modified by shinhs95
	// 기존 "hide-on-mobile" 속성만 있었을 떄의 코드
//	var sortedChildren = this._getSortedChildren();
//	
...
//	});
	...
	for (var idx = 0; idx < visibleChildren.length; idx++) {
		...
		if ((this._container.getAppInstance().targetScreen.name == "mobile" && (this._container.userAttr(ATTR_MOBILE_COLUMN_COUNT) == 1) || this._container.getAppInstance().targetScreen.name == "tablet" && this._container.userAttr(ATTR_TABLET_COLUMN_COUNT) == 1)) {
